    ...
    <div>
      <Field label="Conversion">
        <select className="select" value={mode} onChange={e => setMode(e.target.value)}>
          <option value="conc">µg/mL → nM (needs MW)</option>
          <option value="bp">bp → kDa (dsDNA)</option>
          <option value="od">OD₆₀₀ → cells/mL</option>
        </select>
      </Field>
      <Field label={mode === 'conc' ? 'Concentration (µg/mL)' : mode === 'bp' ? 'Length (bp)' : 'OD₆₀₀'}>
      ...
      {mode === 'conc' && !isNaN(v) && !isNaN(w) && w > 0 && (
        <div className="result-box">{fmt(v)} µg/mL = {fmt((v / w) * 1e6)} nM = {fmt((v / w) * 1e3)} µM = {fmt(v / w)} mM</div>
      )}
      {mode === 'bp' && !isNaN(v) && (
